...
if (T) {
  option_list <- list(
    ...
    make_option(c("-o", "--output_matrix"), type = "character", default = "depth_enrichment.txt", action = "store",
                help = "The output file name for signal enrichment level and sequencing depth matrix [default %default]")
  )
}
...
if (F) {
  xlab <- "sample.name"
  group <- "sample_group_k562.txt"
  depth <- "k562_multiqc_samtools_flagstat.txt"
  enrich <- "k562_deeptools_plot_fingerprint_with0_metrics.txt"
}
...
if (T) {
  ...
  colnames(depth)
  # SE: mapped_passed (10); PE:properly paired_passed (20)
  dep <- ifelse(grepl("_SE.stat$", depth$Sample), depth[, 10], depth[, 20])
  dep <- as.numeric(dep)
  depth <- data.frame(Sample = depth$Sample,
                      total_passed = dep)
}
...
if (exists("xlab")) {
  ...
  ggdat$xlab <- factor(xlab, levels = sort(unique(xlab)))
}
...
if (T) {
  ...
  if (T) {
    ...
    if (exists("xlab")) {
      p <- ggplot(data = ggdat) + 
        geom_bar(aes(x = xlab, y = total_passed, fill = Number), stat = "identity") + 
        scale_x_discrete(name = "") + 
        scale_y_continuous(name = "Number of Reads") + 
        facet_wrap(~Group, nrow = 4, scales = "free_x") + 
        theme(axis.text.x = element_text(size = rel(1.2), angle = 30, hjust = 1, vjust = 1),
    ...
    ggsave(plot = p, filename = output_depth, width = 15,height = 8)  
  }
  ...
  if (T) {
    ...
    if (exists("xlab")) {
      p <- ggplot(data = ggdat) + 
        geom_bar(aes(x = xlab, y = Synthetic.JS.Distance, fill = Number), stat = "identity") + 
        scale_x_discrete(name = "") + 
        scale_y_continuous(name = "Enrichment level") + 
        facet_wrap(~Group, nrow = 4, scales = "free_x") + 
        theme(axis.text.x = element_text(size = rel(1.2), angle = 30, hjust = 1, vjust = 1),
    ...
    ggsave(plot = p, filename = output_enrich, width = 15,height = 8)
  }
}

# output matrix
if (T) {
  write.table(ggdat[, c(1,5,4,2,3)], file = output_matrix, quote = F, sep = "\t", col.names = T, row.names = F)
}
