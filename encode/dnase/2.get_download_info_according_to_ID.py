				...
				for x in replicates:
					...
					if rep_id_ck == rep_id:
						if x['status'] == "archived" or x['status'] == "released":
							...
							# Experiment_info
							result = list()
							if ("adapters" in x['library'].keys()):
								if len(x['library']['adapters'])>0:
									for adapter in x['library']['adapters']:
										result.append(adapter['type'] + ":" + adapter['sequence'])
									result = ";".join(result)
								else:
									result = "NA"
							else:
