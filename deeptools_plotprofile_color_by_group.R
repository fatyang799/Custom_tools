...
if (T) {
  option_list <- list(
    ...
    make_option(c("-g", "--group"), type = "character", default = NULL, action = "store",
                help = "A file recording the type of input files, e.g. ATAC, RNA, et al. [default %default]"),
    
    make_option(c("-G", "--group2"), type = "character", default = NULL, action = "store",
                help = "Additional file recording the type of input files, used to color line of profile [default %default]"),
...
# Analysis parameters
if (T) {
  usage_message <- "Rscript deeptools_plotprofile_color_by_group.R -i file.txt -t -f -o TSS_freeY.png -g group1.txt\nRscript deeptools_plotprofile_color_by_group.R -i file.txt -o Body_fixedY.png -g group1.txt -G group2.txt"
...
if (T) {
  ...
  if (T) {
    colnames(ggdat)
    p <- ggplot(data = ggdat) +
      geom_line(aes(x=Bin, y=Value, group = File, colour = group2), linewidth=0.8) +
